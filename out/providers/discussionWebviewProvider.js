...
class DiscussionWebviewProvider {
    ...
    _currentDiscussion;
    _currentUserName = '';
    constructor(_extensionUri, yamlStorage, gitService, anchorIndexer, treeDataProvider, readMentionsStorage) {
    ...
    async _markCommentMentionRead(commentId) {
        if (!this._currentDiscussion) {
            return;
        }
        try {
    ...
    async _markAllMentionsRead() {
        ...
        try {
            ...
            for (const comment of this._currentDiscussion.comments) {
                if ((0, discussion_js_1.hasMentionFor)(comment.body, this._currentUserName)) {
    ...
    async _addComment(text) {
        ...
        try {
            ...
            const commentId = (0, discussion_js_1.generateCommentId)();
            ...
            await this.gitService.commitDiscussion(currentDiscussion.id, 'Add comment', commentId);
            // Update local state
