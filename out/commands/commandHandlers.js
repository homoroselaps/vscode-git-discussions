...
class CommandHandlers {
    ...
    async createDiscussion() {
        ...
        try {
            ...
            const anchorComment = (0, discussion_js_1.formatAnchorComment)(languageId, discussionId);
            ...
            // Create discussion
            const discussion = (0, discussion_js_1.createNewDiscussion)(discussionId, title, anchor, user.name, initialComment);
            ...
            await this.gitService.commitDiscussion(discussionId, 'Add', `${anchor.file_path}:${startLine}`);
            // Refresh
    ...
    async addComment(discussion) {
        ...
        try {
            ...
            const updatedDiscussion = (0, discussion_js_1.addCommentToDiscussion)(currentDiscussion, user.name, commentText);
            // Write updated YAML
            ...
            await this.gitService.commitDiscussion(discussion.id, 'Add comment', `comment #${newCommentId}`);
            // Refresh
            await this.treeDataProvider.refresh();
     ...
     * Close a discussion
     */
    async closeDiscussion(discussion) {
        if (!this.sidecarService.isLinked) {
            this.sidecarService.showStatusNotification();
            return;
        }
     ...
     */
    async closeAndRemoveAnchor(discussion) {
        if (!this.sidecarService.isLinked) {
            this.sidecarService.showStatusNotification();
            return;
        }
                    ...
                    if (anchorLine >= 0 && anchorLine < document.lineCount) {
                        ...
                        if (pattern.test(line.text)) {
                            await editor.edit(editBuilder => {
                                ...
                                const range = new vscode.Range(anchorLine, 0, anchorLine + 1, 0);
                                editBuilder.delete(range);
                            });
                            await document.save();
                            vscode.window.showInformationMessage(`Discussion ${discussion.id} closed and anchor removed. Please commit your code changes.`);
                        }
                    }
                }
            }
